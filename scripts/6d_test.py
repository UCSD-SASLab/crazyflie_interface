...
DEFAULT_NUM_DRONES = 1  # DeepReach requires 2 drones
...
MODEL_PATH = "/mounted_volume/gym-pybullet-drones/gym_pybullet_drones/examples/drone_6d.pth"
...
class DeepReachController:
    ...
    def __init__(self, model_path=MODEL_PATH):
        ...
        try:
            ...
            self.dynamics = Drone6DWithDist(disturbance_max=1.0, thrust_max=12.0, set_mode='avoid')
    ...
    def compute_control(self, states, dt):
        ...
        # Process single drone with DeepReach
        if len(states) != 1:
            raise ValueError("DeepReach mode only supports 1 drone")
        
        # Extract position and velocity for single drone
        robot_state = states[0]
        pos = robot_state[0:3]  # x, y, z
        vel = robot_state[3:6]  # vx, vy, vz
        
        # Construct 6D state: [x, vx, y, vy, z, vz] (interleaved format)
        ...
        deepreach_input = torch.cat([time_tensor, drone_6d_state_tensor]).unsqueeze(0)  # [1, 7]
        ...
        # Use gradient to compute optimal control
        ...
        current_drone1_state = drone_6d_state   # Single drone state: [x, vx, y, vy, z, vz]
        ...
        target_positions = np.array([integrated_pos1])  # Single drone
        target_velocities = np.array([integrated_vel1])  # Single drone
...
def run(
    ...
    H_STEP = 0.5
    INIT_XYZS = np.array([[0.25, 0, 0.5]])  # Single drone
    ...
    for i in range(0, int(duration_sec*env.CTRL_FREQ)):
        ...
        if mode == "deepreach":
            ...
            for j in range(num_drones):
                ...
                #target_pos = current_pos + np.array([0.05, 0.0, 0.0])  # Move only in x direction
                ...
                if i % 50 == 0:
                    ...
                    print(f"Drone {j} - Target pos: {target_pos}")
                    print(f"Drone {j} - Target vel: {target_vel}")
